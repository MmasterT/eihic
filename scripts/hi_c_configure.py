...
class HI_CCONFIGURE:
    ...
    def __init__(self, args):
        self.args = args
        self.args.sample_csv = HI_CCONFIGURE.check_exits(self.args.samples_csv)
...
# Save the data to a list: R1 forward reads, R2 reverse reads, and sample name/ organism name
        data["R1"] = file[0]
        data["R2"] = file[1]
        data["reference"] = self.args.reference
        data["organism"] = self.args.organism

# Forward reads and sample reads should be =
        if len(data["R1"].items()) != len(data[1].items()):
        ...
        for sample in data["R1"]:
            ...
            if not Path(sample).is_file():
                raise ValueError(
                    f"Sample '{sample}' R1 path does not exist."
            )
        for sample in data["R1"]:
        ...
        # link the reads
        reads_dir = Path(self.args.output).joinpath("reference/reads")
        Path(reads_dir).mkdir(parents=True, exist_ok=True)

        # r1_cmd = f"cd {reads_dir} && ln -s {data["R1"]} x[SID].R1.{ext}"
        for sample in range(len(R1)):
            r1_name = data["R1"]
            r2_name = data["R2"]
            r1_path = Path(reads_dir).joinpath(r1_name)
            r2_path = Path(reads_dir).joinpath(r2_name)
            ...
            Path(r1_path).symlink_to(data["R1"])
            Path(r2_path).symlink_to(data["R2"])
